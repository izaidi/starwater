  ...
  function(Vector,Field,Emitter,Particle){
    ...
    _.extend(ParticleSystem.prototype, Backbone.Events, {
      ...
      onMouseDown : function (evt) {
        ...
        var object;
        if (Attractor.fields.hasOwnProperty(evt.currentTarget.id)) {
          object = Attractor.fields[evt.currentTarget.id];
        } else {
          object = this.getObjectAtPoint(this.mouseCoords);
        }
        console.log(object);
        if (this.selected) {
      ...
      onMouseMove : function (evt) {
        var center = Attractor.getCenter(evt.currentTarget.id);
        if (center != null) {
          this.mouseCoords = new Vector(center.x, center.y);
        } else {
          this.mouseCoords = new Vector(evt.pageX || (evt.layerX - this.display.canvas.offsetLeft), evt.pageY || (evt.layerY - this.display.canvas.offsetTop));
        }
